import numpy as np
from final_fantasy_gridworld import FinalFantasyGridWorld
...
class Ulysses:
    ...
    def q_learn_environment(self, visible_environment):
        ...
        self.env = FinalFantasyGridWorld(visible_environment, self.actions)
        self.Q = self.learn_q()

    '''
    Resets agent's location in environment
    '''
    ...
    def learn_q(self, render=False):
        ...
        for episode in range(self.episodes):
            ...
            s = self.reset_environment()

            # a is an action selected under the epsilon-greedy policy
            # Epsilon greedy only done on first step?
            ...
            while t < self.max_steps:
                ...
                source code for step: https://github.com/openai/gym/blob/master/gym/envs/toy_text/discrete.py
                '''
    ...
    '''
    def get_q_plan(self, render_all=False):
        action_plan = []
        ...
        while True:
            ...
            if moves == 0:
                ...
                self.env.render()
            moves += 1
            a = self.epsilon_greedy_action_selection(self.Q, n_actions, s, train=True)
...
if __name__ == '__main__':
    # A representation of the shrine of fiends environment used for the testing of this agent
    vis_env = [
    ...
    agent = Ulysses()
    agent.q_learn_environment(vis_env)
    agent.get_q_plan(render_all=True)
