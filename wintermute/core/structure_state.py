...
'''
Methods used for both agent and CNN for state identification.

No pressing need to coalesce into standalone class at present
'''


def get_pixel_row_representation(screen_data_capture):
    """
    The screen data capture is a raw string. The nes drew graphics by essentially going pixel by pixel across the
    screen from top.
    ...
    """
    scanline_rows = np.array(
        [screen_data_capture[i - 256:i] for i in range(len(screen_data_capture) + 1) if i % 256 == 0 and i > 0],
    )
    ...
    """
    return scanline_rows[8:-8]


def create_pixel_grid(screen_data_capture):
    scanline_rows = get_pixel_row_representation(screen_data_capture)

    """
    This code annoys me. Basically, we want to find all the x and y points which will delineate the shape of the state
    space. These breakpoints will be combined against the trimmed, formatted, scanline row array.

    It is worth noting the relationship between the nested arrays for x and y. This has made me utterly miserable because 
    I cannot
    keep it straight,
    [
    ...
    the *_up_range variables are given an additional 1 to include the final breakpoint. Quirk of range function. Being 
    python 2.7, unwilling to look for a fix

    The breaks are pretty consistent, we then tabulate them into two lists. I have the feeling these can be coalesced 
    into tuples, but am unwilling to rethink this now
    """
    ...
    column and row sizes are bounded. Until full confidence check, unsure of whether this is fucking with the readout
    """
    grid = []
    for y in y_breaks:
        if len(grid) < 13:
...
def reduce_pixel_square(target):
    try:
        return np.argmax(np.bincount(np.array([np.argmax(np.bincount(np.array(row).astype('int64'))) for row in target if len(row) > 0])))
    except TypeError:
        print('type error')
        for row in target:
            print(row)
